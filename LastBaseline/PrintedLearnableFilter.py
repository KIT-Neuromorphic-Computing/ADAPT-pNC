...
class LearnableFilter(torch.nn.Module):
    ...
    def SampleBeta(self):
        self.coupling = 1. + \
            torch.rand([]).to(self.DEVICE) * self.args.coupling_factor
...
class FilterGroup(torch.nn.Module):
    def __init__(self, args, N_filters, random_state=True):
        ...
        for n in range(N_filters):
            ...
            init_beta = torch.log(torch.tensor(
                init_beta / (1 - init_beta))).to(self.DEVICE)
            self.FilterGroup.append(LearnableFilter(
                args, init_beta, random_state))
    ...
    def __init__(self, args, N_channel, random_state=True):
        ...
        for n in range(N_channel):
            self.FilterGroups.append(
                FilterGroup(args, N_channel, random_state))
...
class pLayer(torch.nn.Module):
    ...
    def MAC(self, a):
        ...
        a_reshaped = a.permute(0, 2, 1)
        ones_tensor = torch.ones(
            [a_reshaped.shape[0], a_reshaped.shape[1], 1]).to(self.device)
        zeros_tensor = torch.zeros_like(ones_tensor).to(self.device)
        a_extend = torch.cat([a_reshaped, ones_tensor, zeros_tensor], dim=2)

        batch_size = a_extend.shape[0]

        a_neg = self.INV(a_extend)
        a_neg[:, :, -1] = torch.tensor(0.).to(self.device)

        positive_w = (
            self.W * positive).unsqueeze(0).expand(batch_size, -1, -1)
        negative_w = (
            self.W * negative).unsqueeze(0).expand(batch_size, -1, -1)
...
class pRecurrentLayer(torch.nn.Module):
    def __init__(self, args, N_in, N_out, ACT, INV):
        ...
        self.model.add_module('2_MAC', pLayer(
            N_out * (N_out + 1), N_out, args, ACT, INV))
...
class PrintedNeuralNetwork(torch.nn.Module):
    def __init__(self, args, N_channel, N_class, N_layer, random_state=True):
        ...
        self.model.add_module('0_pLayer', pRecurrentLayer(
            self.args, N_channel, N_class, self.ACT, self.INV))
